...
class MultitaskingModel:
    ...
    def _generate_processes(self):
        ...
        #                                                      range(self.num_dimensions)):
            ...
            #
            # input_to_hidden_proc = pnl.Process(pathway=[self.input_layers[input_index],
            #                                             input_to_hidden,
            #                                             self.hidden_layer],
            #                                    name='input-{i}-output-{o}-proc'.format(i=input_index,
            #                                                                            o=output_index),
            ...
            #                                                                   2, -1) * self.weight_init_scale)
            #
            # hidden_to_output_proc = pnl.Process(pathway=[self.input_layers[input_index],
            #                             input_to_hidden,
            #                             self.hidden_layer,
            #                             hidden_to_output,
            #                             self.output_layers[output_index]],
            ...
            #                    learning=pnl.ENABLED)
            #
            # self.input_output_processes.append(input_to_hidden_proc)
        ...
        for index in range(self.num_dimensions):
            input_to_hidden = pnl.MappingProjection(name='input-{i}-to-hidden'.format(i=index),
                                                    sender=self.input_layers[index],
                                                    receiver=self.hidden_layer,
                                                    matrix=pnl.random_matrix(self.num_features, self.hidden_layer_size,
                                                                             2, -1) * self.weight_init_scale)

            input_to_hidden_proc = pnl.Process(pathway=[self.input_layers[index],
                                                        input_to_hidden,
                                                        self.hidden_layer],
            ...
            hidden_to_output = pnl.MappingProjection(name='hidden-to-output-{o}'.format(o=index),
                                                     sender=self.hidden_layer,
                                                     receiver=self.output_layers[index],
                                                     matrix=pnl.random_matrix(self.hidden_layer_size, self.num_features,
                                                                              2, -1) * self.weight_init_scale)

            hidden_to_output_proc = pnl.Process(pathway=[self.hidden_layer,
                                                         hidden_to_output,
                                                         self.output_layers[index]],
                                                name='hidden-to-output-{o}-proc'.format(o=index),
                                                learning=pnl.ENABLED)

            self.input_output_processes.append(hidden_to_output_proc)
