...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  6 10:47:12 2017

@author: norbert
"""

#%% Import net structure

import pickle

with open('singleTasks.pickle', 'rb') as handle:
    [units_perdim, input_dim, output_dim, IH_log, HO_log, TH_log, TO_log,
     bias_H, bias_O] = pickle.load(handle)
    
ref_epoch = -1
    
w_IH = IH_log[ref_epoch]
w_HO = HO_log[ref_epoch]
w_TH = TH_log[ref_epoch]
w_TO = TO_log[ref_epoch]
...
tasks, task_map = get_multiTasks(input_dim, output_dim, simult = simult)
...
congruency_log = [] # Congruent Trials
incongruency_log = [] # Incongruent Trials
noncondition_log = [] # Neither conguent nor incongruent (middle ground)
...
good_log = []               # Good/Bad Classifications (abs error < 0.35 across nodes)
...
while train:
    ...
    target_patterns, task_patterns, io_map, patterns = get_trainPatterns(in_patterns, tasks_perm, units_perdim, output_dim, task_map, mult = True)
    
    if congruency:
        
        congruency_map, incongruency_map = getCongruency(patterns, task_patterns, task_map, units_perdim)
        
        congruency_log += [congruency_map]
        incongruency_log += [incongruency_map]
        
        noncondition_map = (congruency_map + incongruency_map) == 0
        noncondition_log += [noncondition_map]
        
        if noncondition:
            
            target_patterns = target_patterns[noncondition_map]
            task_patterns = task_patterns[noncondition_map]
            patterns = patterns[noncondition_map]
        
        elif incongruent:
            
            if flip:
            
                incongruency_map = ~incongruency_map
                
            target_patterns = target_patterns[incongruency_map]
            task_patterns = task_patterns[incongruency_map]
            patterns = patterns[incongruency_map]
            
        else:
            
            if flip:
                
                congruency_map = ~congruency_map
                
            target_patterns = target_patterns[congruency_map]
            task_patterns = task_patterns[congruency_map]
            patterns = patterns[congruency_map]
    ...
    (mae, rmse, pgc, classification, good, expected, choice, mae_byTask, rmse_byTask, 
     pgc_byTask, class_byTask, good_byTask, expected_byTask, choice_byTask) = performance(target_patterns, test_output, task_patterns, mult = True, simult = simult)
    ...
    # Need update performance function
...
#%% Check Hidden Activity

input_patterns = get_inputPatterns(units_perdim, input_dim, sc = False)

tasks, task_map = get_singleTasks(input_dim, output_dim)

target_patterns, task_patterns, io_map, patterns = get_trainPatterns(input_patterns, tasks, units_perdim, output_dim, task_map, mult = False)

ref_epoch = -1

weights = [w_IH_log[ref_epoch], w_TH_log[ref_epoch], w_HO_log[ref_epoch], 
           w_TO_log[ref_epoch], bias_H, bias_O]    # could use any weights across epochs
...
task_node = np.where(task_patterns == np.max(tasks))[1]
...
plt.figure('Hidden Representation Similarity MT')
